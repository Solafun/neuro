...
- КРИТИЧНО: ВСЕ значения, термины и текст должны быть СТРОГО НА РУССКОМ ЯЗЫКЕ. Никаких английских слов (например, ancor, low, high, fight, anxious). АБСОЛЮТНО ВСЕ термины, названия эффектов и искажений переводи на русский язык! Язык: СТРОГО РУССКИЙ. Не используй латиницу внутри русских слов (никакого транслита или смешивания символов). Все термины должны быть на чистой кириллице.
- КРИТИЧНО (ПОЛОВЫЕ ТРЕБОВАНИЯ): - Если пол можно уверенно определить — используй "он" или "она", если нет — используй нейтральные формулировки без местоимений. Не смешивай "он/она" в одном предложении
...

{
  ...
  "system_verdict": {
    "truth_bomb": "ЖЕСТКИЙ ИТОГ. То, что человек боится признать, но что на самом деле определяет его жизнь.",
    "main_conflict": "ВНУТРЕННИЙ РАЗРЫВ. Столкновение двух противоположных базовых потребностей.",
    "self_sabotage": "АКТИВНОЕ СОПРОТИВЛЕНИЕ. Каким конкретным действием он сам ломает свои успехи."
  },
  ...
  "cognitive_profile": {
    ...
    "biases": ["КОГНИТИВНЫЕ ИСКАЖЕНИЯ. 3-4 ошибки из классического списка с нормальным обоснованием! Формат СТРОГО: «<Название на русском>: <как именно это проявляется у этого человека>». Язык: СТРОГО РУССКИЙ. Не используй латиницу внутри русских слов (никакого транслита или смешивания символов). Все термины должны быть на чистой кириллице. Пример: «Эффект якоря: судит людей по первому впечатлению и не меняет мнение, глубокий психоанализ, без воды, Формат: СТРОГО JSON. Не добавляй никакого текста до или после JSON»" ],

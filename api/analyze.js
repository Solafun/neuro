...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Fetch user status first to check for Admin bypass and determine schema tier
    if (telegramId) {
      try {
        const status = await checkUserStatus(telegramId);
        isPaid = status.isPaid;
        isAdmin = status.isAdmin;
        console.log(`User Status: id=${telegramId}, isPaid=${isPaid}, isAdmin=${isAdmin}`);
      } catch (e) {
        console.warn("Failed to check optional user status:", e.message);
      }
    }

    // Check if user is explicit admin via ENV (overrides isAdmin from DB)
    if (telegramId && process.env.ADMIN_TELEGRAM_ID && String(telegramId) === String(process.env.ADMIN_TELEGRAM_ID)) {
      isAdmin = true;
      console.log("Recognition: User is ENV Admin");
    }
    ...
    if (data.posts.length === 0) {
      return res.status(200).json({
        error: 'no_posts',
        message: 'В этом профиле нет публикаций для анализа.'
      });
    }
...
- Не ставь диагнозы
- Не фантазируй без данных. Если данных для конкретного блока абсолютно нет — напиши 'Недостаточно данных для анализа'.
- КРИТИЧНО: Избегай шаблонных психологических выводов. Найди УНИКАЛЬНУЮ черту этого человека.
- Пиши простым языком, но сохраняй глубину.
- КРИТИЧНО: ВСЕ значения и текст СТРОГО НА РУССКОМ ЯЗЫКЕ.
- ВСЕГДА используй правильный род (он или она).
- КРИТИЧНО: ЗАПРЕЩЕНО использовать двойные кавычки (") внутри текстовых значений JSON. Только ОДИНАРНЫЕ (') или ёлочки (« »).
...
`;
    ...
    const schemaCore = `
  "profile_summary": {
    "psychotype": "СТАТИЧНЫЙ ЯРЛЫК (например: «Профессиональный Архитектор»).",
    "summary": "Кто этот человек, его суть личности (3-4 предложения).",
    "core_pattern": "ДИНАМИЧЕСКИЙ ПАТТЕРН. Главная поведенческая петля."
  },
  "positive_core": {
    "natural_strengths": "ПОДРОБНОЕ ОПИСАНИЕ: Врожденные качества (энергия, фокус, склад ума), которые даны от природы. Минимум 2 предложения.",
    "strengths": ["список из 4 ключевых качеств (СИЛЫ) одним словом каждое"],
    "real_world_value": "ПРАКТИЧЕСКИЙ ВЫХОД. В каких именно ситуациях эта природа дает реальное преимущество.",
    "unique_trait": "РЕДКИЙ АКЦЕНТ. То, что реально отличает этого человека от других.",
    "strength_chart": [
      { "name": "Название из списка", "value": "Оценка 0-100" }
    ]
  },
  "system_verdict": {
    "truth_bomb": "ЖЕСТКИЙ ИТОГ. Главная неудобная правда о человеке.",
    "main_conflict": "ВНУТРЕННИЙ РАЗРЫВ. Конкретное столкновение двух потребностей.",
    "self_sabotage": "АКТИВНОЕ СОПРОТИВЛЕНИЕ. Как он сам себе мешает."
  },
  "cognitive_profile": {
    "thinking_style": "СТРУКТУРА МЫСЛИ. Как мозг обрабатывает информацию.",
    "decision_logic": "АЛГОРИТМ ВЫБОРА. На что опирается при выборе."
  },
  "emotional_profile": {
    "core_emotions": ["3-4 доминирующих чувства"],
    "regulation": "ЭМОЦИОНАЛЬНЫЙ ФИЛЬТР. Как управляет чувствами.",
    "stress_response": "РЕАКЦИЯ НА УГРОЗУ. Тип реакции и описание поведения."
  },
  "behavior_profile": {
    "patterns": [
      "ПОВЕДЕНЧЕСКИЙ ЦИКЛ: ситуация → реакция → результат"
    ],
    "life_strategy": "ГЛОБАЛЬНАЯ СТРАТЕГИЯ. Контроль / Избегание / Борьба / Адаптация."
  },
  "weak_zones": {
    "risks": ["ТОП-3 риска личности и возможных срывов (СТРОГО 3 ПУНКТА)"],
    "vulnerabilities": ["2-3 психологические уязвимости"],
    "triggers": ["что выводит человека из равновесия"]
  }
`;

    // Premium schema parts
    const schemaPremium = `
    ,"biases": ["КОГНИТИВНЫЕ ИСКАЖЕНИЯ. 3-4 ошибки с обоснованием."],
    "blind_spots": "ОШИБКИ ВОСПРИЯТИЯ. То, что человек не видит в себе."
  },
  "social_profile": {
    "communication_style": "СТИЛЬ ОБЩЕНИЯ. Прямота, манипуляция, закрытость.",
    "attachment": "ТИП ПРИВЯЗАННОСТИ. Надежный, избегающий, тревожный.",
    "trust_issues": "УРОВЕНЬ ДОВЕРИЯ. Степень открытости.",
    "social_mask": "СОЦИАЛЬНАЯ РОЛЬ. Кем хочет казаться."
  },
  "dark_profile": {
    "manipulation": "Уровень и методы влияния.",
    "toxicity": "Как человек отравляет среду (если выражено).",
  ...
  "development_plan": {
    "growth_points": ["конкретные точки роста"],
    "what_to_change": "что именно нужно изменить",
...
${postsText || 'Посты не найдены.'}`;
    ...
    // --- ТРЕКИНГ СТАТИСТИКИ ---
    try {
      console.log(`Log stats: nickname=@${nickname}, telegramId=${telegramId}, isPaid=${isPaid}`);
      ...
      console.warn("Failed to log check stats:", e);
    }
